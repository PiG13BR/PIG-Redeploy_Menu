...
_redeployMenuDisplay displayAddEventHandler ["KeyDown", {
	private _escPressed = false;
	if (PIG_REDEPLOY_playerInRedeploy isNotEqualTo []) then {
		_escPressed = (_this # 1) isEqualTo 1
	};
	_escPressed
}];
...
// Map focus
(displayCtrl 8851) ctrlMapAnimAdd [0.7, 0.1, (getPosWorld PIG_REDEPLOY_object)];
...
_redeployButton ctrlAddEventHandler ["ButtonClick", {
	...
	_player setVehiclePosition [getPosASL PIG_REDEPLOY_object, [], 3, "NONE"];
...
_redeployMap ctrlAddEventHandler ["Draw", {
	...
	if ((isNull PIG_REDEPLOY_object) || {!alive PIG_REDEPLOY_object}) then {systemChat "ignoring"; continue};
